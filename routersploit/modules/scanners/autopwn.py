...
class Exploit(Exploit):
    ...
    def run(self):
        ...
        self.creds = []
        self.not_verified = []
        print('dang chay, ', self.target)
        # vulnerabilities
        #print()
        print("\033[94m[*]\033[0m", "Starting vulnerablity check...".format(self.target))
        ...
        # default creds
        #print()
        print("\033[94m[*]\033[0m", "{} Starting default credentials check...".format(self.target))
        modules = []
        ...
        if self.not_verified:
            print("\033[94m[*]\033[0m", "{} Could not verify exploitability:".format(self.target))
            for v in self.not_verified:
                print(" - {}:{} {} {}".format(*v))
            print()

        if self.vulnerabilities:
            print("\033[92m[+]\033[0m", "{} Device is vulnerable:".format(self.target))
            headers = ("Target", "Port", "Service", "Exploit")
            print_table(headers, *self.vulnerabilities)
            print()
        else:
            print("\033[91m[-]\033[0m", "{} Could not confirm any vulnerablity\n".format(self.target))

        if self.creds:
            print("\033[92m[+]\033[0m", "{} Found default credentials:".format(self.target))
            headers = ("Target", "Port", "Service", "Username", "Password")
            print_table(headers, *self.creds)
            print()
        else:
            print("\033[91m[-]\033[0m", "{} Could not find default credentials".format(self.target))
    ...
    def exploits_target_function(self, running, data):
        ...
        #            exploit.target_protocol = "custom"
                ...
                if response is True:
                    print("\033[92m[+]\033[0m", "{}:{} {} {} is vulnerable".format(
                    ...
                    print("\033[91m[-]\033[0m", "{}:{} {} {} is not vulnerable".format(
                               exploit.target, exploit.port, exploit.target_protocol, exploit))
                else:
                    print("\033[94m[*]\033[0m", "{}:{} {} {} Could not be verified".format(
                               exploit.target, exploit.port, exploit.target_protocol, exploit))
